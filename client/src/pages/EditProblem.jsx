...
function EditProblem() {
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      try {
        ...
        // Ensure inputs are arrays of objects for useFieldArray
        const constraints = data.constraints?.length 
          ? data.constraints.map(value => ({ value })) 
          : [{ value: "" }]
          
        const hints = data.hints?.length 
          ? data.hints.map(value => ({ value })) 
          : [{ value: "" }]
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await api.put(`/api/v1/problems/${id}`, payload)
